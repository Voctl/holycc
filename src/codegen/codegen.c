...
static void codegen_emit_runtime_protos(CodeGen *cg) {
  string_buffer_append_cstr(
      ...
      "int64_t MemCompare(const uint8_t *a, const uint8_t *b, uint64_t "
      "count);\n"
...
static bool codegen_expr_has_no_side_effects(AstNode *node) {
  if (!node)
    return true;
  switch (node->kind) {
  ...
  case AST_BINARY_EXPR: {
    ...
    if (op == TOK_ASSIGN || op == TOK_PLUS_ASSIGN || op == TOK_MINUS_ASSIGN ||
        ...
        op == TOK_PERCENT_ASSIGN || op == TOK_AMPERSAND_ASSIGN ||
        op == TOK_PIPE_ASSIGN || op == TOK_CARET_ASSIGN ||
        op == TOK_LSHIFT_ASSIGN || op == TOK_RSHIFT_ASSIGN) {
      return false;
    }
    AstNode *left = node->first_child;
    AstNode *right = left ? left->next : NULL;
    return codegen_expr_has_no_side_effects(left) &&
           codegen_expr_has_no_side_effects(right);
  }
  case AST_UNARY_EXPR: {
    TokenKind op = node->data.token_kind;
    // Pre/post increment and decrement have side effects
    if (op == TOK_INCREMENT || op == TOK_DECREMENT)
      return false;
    return codegen_expr_has_no_side_effects(node->first_child);
  }
  case AST_CONDITIONAL_EXPR: {
    AstNode *cond = node->first_child;
    AstNode *then_b = cond ? cond->next : NULL;
    AstNode *else_b = then_b ? then_b->next : NULL;
    return codegen_expr_has_no_side_effects(cond) &&
           codegen_expr_has_no_side_effects(then_b) &&
           codegen_expr_has_no_side_effects(else_b);
  }
  // Calls, assignments embedded in cast/index, etc. — assume side effects
  default:
    return false;
  }
}
...
static void codegen_emit_expr(CodeGen *cg, AstNode *node) {
  ...
  case AST_CHAR_LITERAL: {
    ...
    if (c == '\\')
      ...
      string_buffer_append_char(&cg->buf, c);
    else
      string_buffer_append_fmt(&cg->buf, "\\x%02x", (unsigned char)c);
    string_buffer_append_char(&cg->buf, '\'');
    break;
  }
  ...
  case AST_BOOL_LITERAL:
    string_buffer_append_cstr(&cg->buf,
                              node->data.int_value ? "true" : "false");
    break;
...
static void codegen_emit_stmt(CodeGen *cg, AstNode *node) {
      ...
      if (is_variadic) {
        ...
        if (last_param) {
          ...
          const char *last_name = (pname && pname->kind == AST_IDENTIFIER)
                                      ...
                                      : NULL;
          codegen_emit_indent(cg);
          string_buffer_append_cstr(&cg->buf, "uint64_t argc = 0;\n");
          codegen_emit_indent(cg);
          string_buffer_append_cstr(&cg->buf, "int64_t argv[64];\n");
          codegen_emit_indent(cg);
          string_buffer_append_cstr(&cg->buf, "va_list __va_args;\n");
          codegen_emit_indent(cg);
          string_buffer_append_fmt(&cg->buf, "va_start(__va_args, %s);\n",
                                   last_name ? last_name : "");
          codegen_emit_indent(cg);
          string_buffer_append_cstr(&cg->buf, "while (argc < 64) {\n");
          cg->indent_level++;
          codegen_emit_indent(cg);
          string_buffer_append_cstr(
              &cg->buf, "argv[argc++] = va_arg(__va_args, int64_t);\n");
          cg->indent_level--;
          codegen_emit_indent(cg);
          string_buffer_append_cstr(&cg->buf, "}\n");
          codegen_emit_indent(cg);
          string_buffer_append_cstr(&cg->buf, "va_end(__va_args);\n");
        }
  ...
  case AST_EXPR_STMT: {
    ...
    if (expr && expr->kind == AST_STRING_LITERAL) {
      ...
      // Wrap side-effect-free expressions with (void)(...) to suppress
      // GCC -Wunused-value warnings for bare arithmetic like (2 + 5);
      bool no_side_fx = codegen_expr_has_no_side_effects(expr);
      if (no_side_fx) {
        string_buffer_append_cstr(&cg->buf, "(void)(");
        codegen_emit_expr(cg, expr);
        string_buffer_append_cstr(&cg->buf, ");\n");
      } else {
        codegen_emit_expr(cg, expr);
  ...
  case AST_UNION_DECL: {
    ...
    string_buffer_append_cstr(
        &cg->buf, (node->kind == AST_UNION_DECL) ? "union" : "struct");
    string_buffer_append_cstr(&cg->buf, " {\n");
    cg->indent_level++;
    ...
    while (iter) {
      if (iter->kind == AST_FUNC_DECL) {
        ...
        string_buffer_append_cstr(&cg->buf, ") ");
        AstNode *body = n->next;
        while (body && body->kind == AST_FUNC_PARAM)
          body = body->next;
  ...
  case AST_ENUM_DECL: {
    ...
    while (enumerator) {
      if (enumerator->kind != AST_ENUMERATOR)
        break;
      codegen_emit_indent(cg);
  ...
  case AST_TRY_STMT: {
    ...
    codegen_emit_indent(cg);
    string_buffer_append_cstr(&cg->buf,
                              "int __ex = setjmp(__holyc_jmp_buf);\n");
    codegen_emit_indent(cg);
